...
async function syncCopyButtonsForSelectedVacancy() {
  ...
  // Показываем блок круглых кнопок при любой выбранной вакансии (не только при успешной загрузке из API)
  setCopyButtonsVisible(true);
  updateCopyButtonsDisabledState();
...
async function showContextForTab() {
  ...
  // Для страниц резюме (rabota.by, hh.ru) и Huntflow показываем тот же блок, что и для LinkedIn
  ...
  const showLinkedIn = ctx === CONTEXT.LINKEDIN || ctx === CONTEXT.OTHER || ctx === CONTEXT.RESUME || ctx === CONTEXT.HUNTFLOW;
  if (showLinkedIn) {
  ...
  if (container) {
    ...
    if (dataEl) dataEl.textContent = url ? `Страница: ${url.replace(/^https?:\/\//, '').split('/')[0]}` : '';
  }
  ...
  if (ctx === CONTEXT.LINKEDIN || ctx === CONTEXT.OTHER || ctx === CONTEXT.RESUME) {
