...
function hasBlacklistLabel(candidateInfo) {
  ...
  return labels.some((l) => {
    ...
    const n = norm(name);
    return n === "черный список" || n === "black list" || n === "blacklist";
  });
}
...
function createFloatingWidget() {
  ...
  toolbarRow.style.cssText = "display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-shrink:0;min-height:0;";
  ...
  headerRight.style.cssText = "display:flex;align-items:center;gap:6px;flex-shrink:0;margin-left:auto;";
  ...
  body.style.cssText = "display:flex;flex-direction:column;gap:8px;flex:1;min-height:0;overflow-y:auto;color:inherit;";
...
function updateFloatingWidgetHeader() {
  ...
  if (STATE.current.mode === "open" && STATE.current.appUrl) {
    ...
    const btnStyle = "padding:6px 10px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;border:none;transition:background .2s;height:32px;display:inline-flex;align-items:center;justify-content:center;";
    ...
    copyBtn.style.cssText = btnStyle + "background:var(--hrhelper-btn-bg,rgba(0,0,0,.06));color:var(--hrhelper-text,#333);padding:6px;";
    ...
    addVacancyBtn.style.cssText = btnStyle + "background:var(--hrhelper-accent,#0a66c2);color:#fff;width:32px;padding:0;";
  ...
  if (toolbarRow) {
    ...
    toolbarRow.style.marginBottom = hasContent ? "12px" : "0";
  }
}
...
function populateFloatingWidgetBody(body) {
  ...
  const { active, rejected, hired } = categorizeVacancies(vacancies);
  ...
  if (hired.length > 0) {
  ...
  } else if (active.length > 0 || rejected.length > 0 || STATE.current.vacancy_name) {
    const selId = STATE.linkedinFull.selectedVacancyId ?? STATE.linkedinFull.defaultVacancyId;
    active.forEach((v) => {
    ...
    if (rejected.length > 0) {
      ...
      rejected.forEach((v) => {
        const card = document.createElement("div");
        card.style.cssText = "padding:8px 10px;margin-bottom:4px;border-radius:6px;font-size:12px;border:1px solid var(--hrhelper-danger-border);background:var(--hrhelper-danger-bg);color:var(--hrhelper-danger);";
...
function appendFloatingCommentsBlock(body) {
  ...
  const expanded = !!floatingWidgetUIState.commentsOpen;
  const header = document.createElement("div");
  ...
  header.innerHTML = '<span class="hrhelper-comments-chevron" style="transition:transform .2s;">▼</span> Последние комментарии';
