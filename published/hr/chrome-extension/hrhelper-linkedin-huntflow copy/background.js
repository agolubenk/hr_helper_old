...
// Для «неизвестной» страницы — иконки в 1.5× размерах (24, 48, 72, 192), подставляются под стандартные ключи
...
/** Возвращает true, если URL — поддерживаемая страница расширения (LinkedIn, резюме, Huntflow, Meet, Calendar). */
function isKnownExtensionHost(url) {
  ...
  try {
    ...
    if (host.includes("calendar.google.com")) return true;
    if (host.includes("meet.google.com")) return true;
    if ((host === "rabota.by" || host === "www.rabota.by" || host.endsWith(".rabota.by") || host.endsWith(".hh.ru")) && path.includes("/resume/")) return true;
    if (host === "hh.ru" && path.includes("/resume/")) return true;
    if (host.includes("huntflow") && (path.includes("/my") || hash.includes("/my")) && (hash.includes("applicants/filter/all") || (hash.includes("applicants") && hash.includes("/id/")) || (hash.includes("vacancy") && hash.includes("/id/")) || (hash.includes("applicants") && /\/filter\/\d+\/\d+/.test(hash)))) return true;
  } catch (_) {}
  return false;
}
...
function updateExtensionIcon(tabId, url) {
  ...
  const useDark = !isKnownExtensionHost(url);
  const path = useDark ? UNKNOWN_PAGE_ICONS : DEFAULT_ICONS;
