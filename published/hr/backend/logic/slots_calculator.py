...
class SlotsCalculator:
    ...
    def __init__(self, work_start_hour: int = 10, work_end_hour: int = 18, 
                 meeting_interval_minutes: int = 15):
        """
        Инициализация калькулятора слотов
        
        Args:
            work_start_hour: Час начала рабочего дня (по умолчанию 10)
    ...
    def calculate_slots_for_two_weeks(self, events_data: List[Dict], 
        ...
        Возвращает до 10 дней: оставшиеся дни текущей недели + следующая неделя.
        В субботу и воскресенье текущая неделя не содержит рабочих дней — берём следующую и через неделю.
        Прошедшие дни пропускаются.
        """
        today = datetime.now(self.minsk_tz)
        # Суббота (5) и воскресенье (6): в текущей неделе нет будущих рабочих дней — сдвигаем на следующую
        if today.weekday() >= 5:
            ...
            next_week_slots = self.calculate_slots_for_week(
                events_data, required_duration_minutes, week_offset=2
            )
        else:
            current_week_slots = self.calculate_slots_for_week(
    ...
    def _calculate_available_slots_for_day(self, day_events: List[Dict], 
        ...
        for event in day_events:
            # События на весь день не создают почасовых занятых интервалов
            if event.get('is_all_day') or event.get('isallday'):
                continue
            start = event.get('start')
            if isinstance(start, dict) and 'dateTime' not in start and 'date' in start:
    ...
    def _parse_event_start(self, event: Dict) -> Optional[datetime]:
        ...
        try:
            ...
            if isinstance(start, dict):
                ...
                return dt.astimezone(self.minsk_tz)
            else:
                start_str = start

            # Парсим ISO формат (ожидается строка с 'T' для даты-времени)
            if 'T' in start_str:
